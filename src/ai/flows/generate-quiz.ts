...
9.  **Impeccable and Robust LaTeX Formatting (RECALL CRITICAL RULE #0 ON WRAPPING ALL MATH IN DOLLAR SIGNS):**
    ...
    *   Units and Special Text in Math: For units or textual descriptions within mathematical expressions, use \`\\\\text{your text}\`. Example: \`\\$v = 25 \\\\text{ m/s}\\$\`. Ensure the text inside \`\\\\text{}\` is simple and does not itself contain complex LaTeX or unescaped special characters. For example, for 'N m^2/C^2', using \`\\\\text{N}\\\\cdot\\\\text{m}^2/\\\\text{C}^2\` or even \`N \\\\cdot m^2/C^2\` (if clearly part of a larger math expression) can be more robust than one large \`\\\\text\` block for the entire unit. ALWAYS ensure the entire mathematical expression, including any \`\\\\text\` components and units, is enclosed within a single pair of dollar-sign delimiters (e.g., \`\\$8.99 \\\\times 10^9 \\\\text{ N} \\\\cdot \\\\text{m}^2/\\\\text{C}^2\\$\`).
