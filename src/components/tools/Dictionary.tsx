...
export function Dictionary() {
  ...
  const handleSearch = async () => {
    ...
    if (lang !== 'en') {
      try {
        const response = await fetch(`https://en.wiktionary.org/w/api.php?action=query&format=json&prop=extracts&titles=${searchTerm}&origin=*`);
        if (!response.ok) {
        ...
        setDefinitions([{ word: searchTerm, meanings: [{ definitions: [{ definition: extract }] }] }]);
        setError(null);
      } catch (err) {
